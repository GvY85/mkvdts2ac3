#!/bin/bash
# mkvPCM2EAC3.sh - add an EAC3 track to an MKV from its PCM track
...
echo "mkvPCM2EAC3-2.0.0 - by Jake Wharton <jakewharton@gmail.com>"
...
# Check for a .mkvPCM2EAC3.rc file in user's home directory for custom defaults
if [ -f ~/.mkvPCM2EAC3.rc ]; then
	. ~/.mkvPCM2EAC3.rc
fi
...
# Usage: displayhelp
	...
	echo "     -t TRACKID,"
	echo "     --track TRACKID  Specify alternate PCM track."
	echo "     -w FOLDER,"
...
while [ -z "$MKVFILE" ]; do
	...
	case "$1" in
		...
		"-i" | "--initial" ) # Make new EAC3 track the first in the file
			INITIAL=1
		;;
		"-k" | "--keep-PCM" ) # Only allow external PCM track if muxing EAC3 track
			if [ -z $EXTERNAL ]; then
				KEEPPCM=1
			fi
		;;
		...
		"--md5" ) #Perform MD5 comparison when copying across drives
			MD5=1
		;;
		"-n" | "--no-PCM" ) # Only allow this if we aren't making the file external
			if [ -z $EXTERNAL ]; then
				NOPCM=1
			fi
		;;
		...
		* )
			...
			if [ $# -ne 0 ]; then
				...
				echo ""
				echo $"Control Flags:"
				printf "  %s: %s" $"Strip PCM:" $NOPCM
				printf "  %s: %s" $"Keep PCM: " $KEEPPCM
				printf "  %s: %s" $"Set EAC3 default: " $DEFAULT
				printf "  %s: %s" $"External EAC3: " $EXTERNAL
				printf "  %s: %s" $"PCM track: " $PCMTRACK
...
# ------ GATHER DATA ------
# If the track id wasn't specified via command line then search for the first PCM audio track
if [ -z $PCMTRACK ]; then
	doprint ""
	doprint $"Find first PCM track in MKV file."
	doprint "> mkvmerge -i \"$MKVFILE\" | grep -m 1 \"${AUDIOTRACKPREFIX}PCM)\" | cut -d ":" -f 1 | cut -d \" \" -f 3"
	PCMTRACK="PCMTRACK" #Value for debugging
	dopause
	if [ $EXECUTE = 1 ]; then
		PCMTRACK=$(mkvmerge -i "$MKVFILE" | grep -m 1 "${AUDIOTRACKPREFIX}PCM" | cut -d ":" -f 1 | cut -d " " -f 3)

		# Check to make sure there is a PCM track in the MVK
		if [ -z $PCMTRACK ]; then
			error $"There are no PCM tracks in '$MKVFILE'."
			exit 1
		fi
	fi
	doprint "RESULT:PCMTRACK=$PCMTRACK"
else
	# Checks to make sure the command line argument track id is valid
	doprint ""
	doprint $"Checking to see if PCM track specified via arguments is valid."
	doprint "> mkvmerge -i \"$MKVFILE\" | grep \"Track ID $PCMTRACK: ${AUDIOTRACKPREFIX}PCM)\""
	VALID=$"VALID" #Value for debugging
	dopause
	if [ $EXECUTE = 1 ]; then
		VALID=$(mkvmerge -i "$MKVFILE" | grep "Track ID $PCMTRACK: ${AUDIOTRACKPREFIX}PCM)")

		if [ -z "$VALID" ]; then
			error $"Track ID '$PCMTRACK' is not a PCM track and/or does not exist."
			exit 1
		else
			info $"Using alternate PCM track with ID '$PCMTRACK'."
		fi
	fi
	doprint "RESULT:VALID=$VALID"
fi
# Get the specified PCM track's information
doprint ""
doprint $"Extract track information for selected PCM track."
...
if [ $EXECUTE = 1 ]; then
	...
	FIRSTLINE=$(echo "$INFO" | grep -n -m 1 "Track number: $PCMTRACK" | cut -d ":" -f 1)
	INFO=$(echo "$INFO" | tail -n +$FIRSTLINE)
	LASTLINE=$(echo "$INFO" | grep -n -m 1 "Track number: $(($PCMTRACK+1))" | cut -d ":" -f 1)
	if [ -z "$LASTLINE" ]; then
...
#Get the language for the PCM track specified
doprint ""
doprint $"Extract language from track info."
doprint '> echo "$INFO" | grep -m 1 \"Language\" | cut -d \" \" -f 5'

PCMLANG=$"PCMLANG" #Value for debugging
dopause
if [ $EXECUTE = 1 ]; then
	PCMLANG=$(echo "$INFO" | grep -m 1 "Language" | cut -d " " -f 5)
	if [ -z "$PCMLANG" ]; then
		PCMLANG=$"eng"
	fi
fi
doprint "RESULT:PCMLANG=$PCMLANG"

# Check if a custom name was already specified
if [ -z $PCMNAME ]; then
	# Get the name for the PCM track specified
	doprint ""
	doprint $"Extract name for selected PCM track. Change PCM to EAC3 and update bitrate if present."
	doprint '> echo "$INFO" | grep -m 1 "Name" | cut -d " " -f 5- | sed "s/PCM/EAC3/" | awk '"'{gsub(/[0-9]+(\.[0-9]+)?(M|K)bps/,"6144kbps")}1'"''
	PCMNAME="PCMNAME" #Value for debugging
	dopause
	if [ $EXECUTE = 1 ]; then
		PCMNAME=$(echo "$INFO" | grep -m 1 "Name" | cut -d " " -f 5- | sed "s/PCM/EAC3/" | awk '{gsub(/[0-9]+(\.[0-9]+)?(M|K)bps/,"6144kbps")}1')
	fi
	doprint "RESULT:PCMNAME=$PCMNAME"
fi
...
doprint "> mkvextract timecodes_v2 \"$MKVFILE\" $PCMTRACK:\"$TCFILE\""
...
if [ $EXECUTE = 1 ]; then
	...
	nice -n $PRIORITY mkvextract timecodes_v2 "$MKVFILE" $PCMTRACK:"$TCFILE"
...
# Extract the PCM track
doprint ""
doprint $"Extract PCM file from MKV."
doprint "> mkvextract tracks \"$MKVFILE\" $PCMTRACK:\"$PCMFILE\""

dopause
if [ $EXECUTE = 1 ]; then
	color YELLOW; echo $"Extracting PCM Track: "; color OFF
	nice -n $PRIORITY mkvextract tracks "$MKVFILE" $PCMTRACK:"$PCMFILE" 2>&1|perl -ne '$/="\015";next unless /Progress/;$|=1;print "%s\r",$_' #Use Perl to change EOL from \n to \r show Progress %
	checkerror $? $"Extracting PCM track failed." 1
	timestamp $"PCM track extracting took:	"
fi

# ------ CONVERSION ------
# Convert PCM to EAC3
doprint $"Converting PCM to EEAC3."
doprint "> ffmpeg -i \"$PCMFILE\" -acodec eac3 -ac 6 -ab 6144k \"$EAC3FILE\""

dopause
if [ $EXECUTE = 1 ]; then
	color YELLOW; echo $"Converting PCM to EAC3:"; color OFF
	PCMFILESIZE=$($DUCMD "$PCMFILE" | cut -f1) # Capture PCM filesize for end summary
	nice -n $PRIORITY ffmpeg -i "$PCMFILE" -acodec eac3 -ac 6 -ab 6144k "$EAC3FILE" 2>&1|perl -ne '$/="\015";next unless /size=\s*(\d+)/;$|=1;$s='$PCMFILESIZE';printf "Progress: %.0f%\r",450*$1/$s' #run ffmpeg and only show Progress %. Need perl to read \r end of lines
	checkerror $? $"Converting the PCM file to EAC3 failed" 1

	# If we are keeping the PCM track external copy it back to original folder before deleting
	if [ ! -z $KEEPPCM ]; then
		color YELLOW; echo $"Moving PCM track to MKV directory."; color OFF
		$RSYNCCMD "$PCMFILE" "$DEST"
		checkerror $? $"There was an error copying the PCM track to the MKV directory. You can perform this manually from \"$PCMFILE\"." 1
	fi
	cleanup "$PCMFILE"
	echo "Progress: 100%"	#The last Progress % gets overwritten so let's put it back and make it pretty
	timestamp $"PCM track conversion took:	"
fi
...
if [ $EXTERNAL ]; then
	...
	# If user doesn't want the original PCM track drop it
	if [ $NOPCM ]; then
		...
		if [ $AUDIOTRACKS -eq 1 ]; then
			# If there is only the PCM audio track then drop all audio tracks
			CMD="$CMD -A"
		else
			# Get a list of all the other audio tracks
			SAVETRACKS=$(mkvmerge -i "$MKVFILE" | grep "$AUDIOTRACKPREFIX" | cut -d ":" -f 1 | grep -vx "Track ID $PCMTRACK" | cut -d " " -f 3 | awk '{ if (T == "") T=$1; else T=T","$1 } END { print T }')
			# And copy only those
	...
	# If the language was set for the original PCM track set it for the EAC3
	if [ $PCMLANG ]; then
		CMD="$CMD --language 0:$PCMLANG"
	fi

	# If the name was set for the original PCM track set it for the EAC3
	if [ "$PCMNAME" ]; then
		CMD="$CMD --track-name 0:\"$PCMNAME\""
	fi

	# If there was a delay on the original PCM set the delay for the new EAC3
	if [ $DELAY != 0 ]; then
...
if [ $EXECUTE = 1 -a $PAUSE = 0 ];then
	...
	printf "%23s %15d KB\n" $"Extracted PCM Filesize:" $PCMFILESIZE|sed -e :a -e 's/\(.*[0-9]\)\([0-9]\{3\}\)/\1,\2/;ta'
