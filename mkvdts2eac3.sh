#!/bin/bash
# mkvdts2EAC3.sh - add an EAC3 track to an MKV from its DTS track
...
echo "mkvdts2EAC3-2.0.0 - by Jake Wharton <jakewharton@gmail.com>"
...
# Check for a .mkvdts2EAC3.rc file in user's home directory for custom defaults
if [ -f ~/.mkvdts2EAC3.rc ]; then
	. ~/.mkvdts2EAC3.rc
fi
...
while [ -z "$MKVFILE" ]; do
	...
	case "$1" in
		...
		"-i" | "--initial" ) # Make new EAC3 track the first in the file
			INITIAL=1
		;;
		"-k" | "--keep-dts" ) # Only allow external DTS track if muxing EAC3 track
			if [ -z $EXTERNAL ]; then
				KEEPDTS=1
			fi
		;;
...
if [ "$(mkvmerge -i "$MKVFILE" | grep -i "${AUDIOTRACKPREFIX}EAC3")" ]; then
...
if [ -z $DTSNAME ]; then
	...
	doprint ""
	doprint $"Extract name for selected DTS track. Change DTS to EAC3 and update bitrate if present."
	doprint '> echo "$INFO" | grep -m 1 "Name" | cut -d " " -f 5- | sed "s/DTS/EAC3/" | awk '"'{gsub(/[0-9]+(\.[0-9]+)?(M|K)bps/,"6144kbps")}1'"''
	DTSNAME="DTSNAME" #Value for debugging
	dopause
	if [ $EXECUTE = 1 ]; then
		DTSNAME=$(echo "$INFO" | grep -m 1 "Name" | cut -d " " -f 5- | sed "s/DTS/EAC3/" | awk '{gsub(/[0-9]+(\.[0-9]+)?(M|K)bps/,"6144kbps")}1')
	fi
...
# ------ CONVERSION ------
# Convert DTS to EAC3
doprint $"Converting DTS to EEAC3."
doprint "> ffmpeg -i \"$DTSFILE\" -acodec EAC3 -ac 6 -ab 6144k \"$EAC3FILE\""

dopause
if [ $EXECUTE = 1 ]; then
	color YELLOW; echo $"Converting DTS to EAC3:"; color OFF
	DTSFILESIZE=$($DUCMD "$DTSFILE" | cut -f1) # Capture DTS filesize for end summary
	nice -n $PRIORITY ffmpeg -i "$DTSFILE" -acodec eac3 -ac 6 -ab 6144k "$EAC3FILE" 2>&1|perl -ne '$/="\015";next unless /size=\s*(\d+)/;$|=1;$s='$DTSFILESIZE';printf "Progress: %.0f%\r",450*$1/$s' #run ffmpeg and only show Progress %. Need perl to read \r end of lines
	checkerror $? $"Converting the DTS file to EAC3 failed" 1
...
if [ $EXTERNAL ]; then
	...
	# If the language was set for the original DTS track set it for the EAC3
	if [ $DTSLANG ]; then
		CMD="$CMD --language 0:$DTSLANG"
	fi

	# If the name was set for the original DTS track set it for the EAC3
	if [ "$DTSNAME" ]; then
		CMD="$CMD --track-name 0:\"$DTSNAME\""
	fi

	# If there was a delay on the original DTS set the delay for the new EAC3
	if [ $DELAY != 0 ]; then
